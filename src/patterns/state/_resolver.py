...
class StateRegistryResolver:
    ...
    def resolve_item_inheritance(
        ...
        # using a queue like this we do not care about the order we get the state-items
        resolved_states = []
        state_items_left_to_resolve = deque(iterable=matched_state_items)
        while state_items_left_to_resolve:
            state = state_items_left_to_resolve.popleft()
            bases_to_inherit_from = [
                base for base in state.cls.__bases__ if base in state_item_by_cls
            ]
            unresolved_inheritance_bases = [
                base for base in bases_to_inherit_from if base not in resolved_classes
            ]
            if unresolved_inheritance_bases:
                state_items_left_to_resolve.append(state)
                continue

            for base in unresolved_inheritance_bases:
                state_to_inherit = state_item_by_cls[base]
                state = state.inherit_from_state(other=state_to_inherit)

            state = state.with_new_internal_state(InternalItemState.resolved)
            resolved_classes.add(state.cls)
            resolved_states.append(state)
            state_item_by_cls[state.cls] = state
